...
這是 **Xime-Enhanced**，在 Xime 上游基礎上增強的 fork 版本，新增以下功能：
...
> **隱私**：簡訊內容僅在本機解析，絕不上傳；簡訊權限為可選授權，可隨時在「管理權限」中撤銷。

## 變更日誌（增強版）

- `2.7.2-enhanced` — 初始增強版：AIDL 語音輸入（bibi/說點啥）；簡訊驗證碼快捷插入與自動複製；目錄選單與設定頁「管理權限」入口。
- `2.7.2-enhanced.2` — 簡訊驗證碼外掛化（sms-code）+ 可設定提取正規表示式；雙拼對照面板 +「先聲母後韻母」即時分解；支援全部雙拼方案動態鍵面；`.xipk`/`.yaml` 檔案關聯；「雙拼提示」開關。
