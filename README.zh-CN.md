...
这是 **Xime-Enhanced**，在 Xime 上游基础上增强的 fork 版本，新增以下功能：
...
> **隐私**：短信内容仅在本机解析，绝不上传；短信权限为可选授权，可随时在「管理权限」中撤销。

## 变更日志（增强版）

- `2.7.2-enhanced` — 初始增强版：AIDL 语音输入（bibi/说点啥）；短信验证码快捷插入与自动复制；目录菜单与设置页「管理权限」入口。
- `2.7.2-enhanced.2` — 短信验证码插件化（sms-code）+ 可配置提取正则；双拼对照面板 +「先声母后韵母」实时分解；支持全部双拼方案动态键面；`.xipk`/`.yaml` 文件关联；「双拼提示」开关。
